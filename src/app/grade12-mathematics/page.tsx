...
import { grade12MathematicsData, Grade12MathematicsTopic, Grade12MathematicsLesson } from '@/data/grade12-mathematics-data';

export default function Grade12Mathematics() {
  const [selectedTopic, setSelectedTopic] = useState<string | null>(null);
  const [completedLessons, setCompletedLessons] = useState<Set<string>>(new Set());
  const [currentTopic, setCurrentTopic] = useState<Grade12MathematicsTopic | null>(null);

  useEffect(() => {
    // Load completed lessons from localStorage
    const saved = localStorage.getItem('completedLessons-grade12-mathematics');
    if (saved) {
    ...
    if (topicId) {
      const topic = grade12MathematicsData.topics.find(t => t.id === topicId);
      if (topic) {
  ...
  const saveProgress = (updatedCompleted: Set<string>) => {
    localStorage.setItem('completedLessons-grade12-mathematics', JSON.stringify([...updatedCompleted]));
  ...
  const calculateTopicProgress = (topic: Grade12MathematicsTopic) => {
  ...
  const calculateOverallProgress = () => {
    const totalLessons = grade12MathematicsData.topics.reduce((sum, topic) => sum + topic.lessons.length, 0);
    const completedCount = grade12MathematicsData.topics.reduce((sum, topic) => 
  ...
  const getTotalDuration = () => {
    return grade12MathematicsData.totalDuration;
  };

  const getCompletedDuration = () => {
    return grade12MathematicsData.topics.reduce((sum, topic) => 
      sum + topic.lessons
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
        <Navigation 
          ...
          subtitle={`Grade 12 Mathematics - ${currentTopic.title}`}
        />
              ...
              return (
                <Card 
                  ...
                  onClick={() => {
                    ...
                    localStorage.setItem('selectedGrade', 'grade12-mathematics');
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="container mx-auto px-4 py-8">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg p-6 shadow-lg">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                ...
                <p className="text-slate-600 dark:text-slate-300">
                  CAPS-aligned curriculum covering advanced calculus, functions, trigonometry, geometry, and statistics
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                  {grade12MathematicsData.topics.length}
                </div>
                <div className="text-sm text-slate-600 dark:text-slate-300">
                  Topics
                </div>
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600 dark:text-green-400">
                  {grade12MathematicsData.topics.reduce((sum, topic) => sum + topic.lessons.length, 0)}
                </div>
                <div className="text-sm text-slate-600 dark:text-slate-300">
                  Lessons
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
          {grade12MathematicsData.topics.map((topic, index) => (
            <Card 
              key={topic.id} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <ul className="text-sm text-slate-600 dark:text-slate-300 space-y-1">
                  ...
                  <li>• Advanced function analysis</li>
                </ul>
              </div>
              <div>
                <h4 className="font-medium text-slate-800 dark:text-slate-100 mb-2">
                ...
                <ul className="text-sm text-slate-600 dark:text-slate-300 space-y-1">
                  ...
                  <li>• Proportionality & similarity</li>
                </ul>
              </div>
              <div>
                <h4 className="font-medium text-slate-800 dark:text-slate-100 mb-2">
                ...
                <ul className="text-sm text-slate-600 dark:text-slate-300 space-y-1">
                  ...
                  <li>• Annuities & decision making</li>
                </ul>
              </div>
            </div>
          </div>
